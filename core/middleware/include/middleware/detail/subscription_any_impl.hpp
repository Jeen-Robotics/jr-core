...
/// Subscription that receives raw bytes and deserializes using protobuf reflection.
/// Requires access to the topic_types registry to look up message types.
class SubscriptionAnyImpl final : public SubscriptionBase {
public:
    using Callback = std::function<void(const std::string&, const google::protobuf::Message&)>;
    ...
    bool spin_once() override {
        ...
        if (!impl_ || !subscriber_valid_impl(impl_.get())) {
            return false;
        }
        
        auto result = subscriber_recv_impl(impl_.get());
        ...
        if (type_full_name != cached_type_name_) {
            ...
            cached_descriptor_ = pool_ ? pool_->FindMessageTypeByName(type_full_name) : nullptr;
        }
        ...
        if (cached_descriptor_ && factory_ && callback_) {
            ...
            if (prototype) {
                ...
                if (msg && msg->ParseFromArray(result.data.data(), 
