...
/// @file middleware.hpp
/// @brief ROS-like middleware API with selectable Rust/C++ backends
///
/// Provides a familiar Node-based API similar to ROS, with automatic
/// message dispatch via background threads.
///
/// @note The Rust backend uses a global singleton (OnceCell), and the C++
///       backend is process-global as well. All Middleware instances share
...
/// Uses the selected middleware backend for message routing.
///
/// @warning SINGLETON BACKEND: All Middleware instances share the same
///          underlying transport backend. Topics are globally visible across
...
public:
  ...
  Subscription subscribe(
    ...
    return do_subscribe_typed<ProtoT>(
      topic,
      type_full_name,
      std::move(callback),
      qos,
  ...
  /// Get topic names and types (C++ registry only, not transport backend)
  std::vector<TopicInfo> get_topic_names_and_types() const;

  /// Register a topic type for subscribe_any() support
  /// Called by Node::create_publisher() to ensure type is available for
  /// BagWriter
  void register_topic_type(
    const std::string& topic,
    const std::string& type_full_name
  );
...
private:
  ...
  // mutable to allow locking in const methods (subscription_valid,
  // get_topic_names_and_types)
  mutable std::mutex mutex_;
  // Use shared_ptr to allow safe concurrent access during dispatch
  std::unordered_map<std::uint64_t, std::shared_ptr<detail::SubscriptionBase>>
    subscriptions_;
  std::unordered_map<std::string, std::string>
    topic_types_; // topic -> type_full_name
  std::unordered_map<std::string, Qos>
    topic_qos_preferences_; // topic -> strongest requested qos
  struct PublisherEntry {
    std::shared_ptr<detail::PublisherHandle> impl;
    Qos qos;
    std::size_t capacity;
  };
  ...
  bool init_failed_{false}; // Set if selected backend initialization fails
  ...
  }; // Persistent epoll instance for O(1) event dispatch
  std::unordered_map<int, std::uint64_t>
    fd_to_id_; // Map eventfd -> subscription id
#endif
};
